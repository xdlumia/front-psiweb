 ...
 * @LastEditors: 赵伦
 * @LastEditTime: 2019-11-10 20:59:46
 * @Description: 采购入库单
*/
<template>
    <sideDetail :status="status" :title="`采购入库单 ${detail?detail.putinCode:''}`" :visible.sync="showDetailPage" @close="close" width="990px">
        ...
        <el-tabs class="wfull hfull tabs-view">
            <el-tab-pane label="详情">
                <el-form size="mini" v-if="detail">
                    ...
                    <commodityInfo :data="detail" disabled id="commodityInfo" />
                    <commodityInfo :data="detail" disabled />
                    <paymentLate :data="detail" disabled id="paymentLate" />
                    <billInfo :data="detail" disabled id="billInfo" />
                    <customInfo :data="detail" disabled id="customInfo" />
                    <extrasInfo :data="detail" disabled id="extrasInfo" />
                </el-form>
            </el-tab-pane>
            <el-tab-pane label="请购单">
                请购单
                <OrderBuying />
            </el-tab-pane>
        ...
        <Edit :rowData="detail" :visible.sync="showEdit" v-if="showEdit" />
    </sideDetail>
</template>
<script>
