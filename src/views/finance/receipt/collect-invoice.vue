    ...
    <d-tabs style="max-height:calc(100vh - 130px)">
      ...
      <div>
        <el-form :model="form" class="p10" ref="form" size="mini" v-if="visible&&form">
          <make-invoice-info :data="form" id="invoice" />
          <make-buyer :data="form" id="buyer" />
          <make-buyer :data="form" from="供应商" id="saler" prefix="market" title="销售方信息" />
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        if (this.isEdit) {
          await this.$api.seePsiFinanceService.finvoicereceivableUpdate(
            this.form
          );
        } else {
          await this.$api.seePsiFinanceService.finvoicereceivableSave(
            this.form
          );
        }
        this.setEdit();
