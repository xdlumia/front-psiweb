 ...
 * @LastEditors: 赵伦
 * @LastEditTime: 2019-11-10 20:28:41
 * @Description: 商品发货信息
*/  
<template>
    <form-card class="borrow-goods" title="商品发货信息">
        <div slot="title">
            <span>商品发货信息</span>
        </div>
        <el-table :data="data.delivers" ref="elTable" row-key="name" size="mini" style="height:100%;">
            ...
            <el-table-column label="序列号" min-width="200" prop="snCode">
                <template slot-scope="{row}">
                    <el-input size="mini" v-model="row.snCode"></el-input>
                </template>
            </el-table-column>
            <el-table-column label="订单号" min-width="200" prop="orderCode">
                <template slot-scope="{row}">
                    <el-input size="mini" v-model="row.orderCode"></el-input>
                </template>
            </el-table-column>
        </el-table>
    </form-card>
