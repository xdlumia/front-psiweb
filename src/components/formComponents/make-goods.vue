...
*/
<template>
  <form-card :class="[(data.invoiceDetailList || []).filter(item=>item.type==0).length?'':'no-goods']" class="invoice-goods" title="发票内容">
    <el-table :data="(data.invoiceDetailList || []).filter(item=>item.type==0)" :summary-method="getSum" show-summary size="mini">
      ...
      <el-table-column label="商品数量" min-width="100" prop="quantity" show-overflow-tooltip>
        <template slot-scope="{$index,row}">
          <el-form-item :prop="`invoiceDetailList.${$index}.quantity`" :rules="[{required:true},{type:'positiveNum'}]" size="mini">
